...
`{\footnotesize`{=latex}
```{python}
#| label: tbl-users-with-games-above-average
#| tbl-cap: Użytkownicy z ponad przeciętną liczbą gier
#| echo: false

import pandas as pd
df = pd.read_csv("users-with-games-above-average.csv")
...
`{\footnotesize`{=latex}
```{python}
#| label: tbl-games-above-category-average
#| tbl-cap: Gry z oceną powyżej średniej w swojej kategorii
#| echo: false

import pandas as pd
df = pd.read_csv("above-average-games.csv")
df.name = "Test name"
df = df.style.hide(axis='index')
df
```
...
`{\footnotesize`{=latex}
```{python}
#| label: tbl-user-game-collection
#| tbl-cap: Kolekcja gier użytkownika casual_gamer
#| echo: false

import pandas as pd
df = pd.read_csv("user_game_collection.csv")
df = df.style.hide(axis='index')
df
```
...
Z dokumentacji @postgres-docs:unique-constraints dowiedziałem się, że `UNIQUE` tworzy indeks B-tree automatycznie. \
